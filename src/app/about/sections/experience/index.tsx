...
function WorkItem({ work }) {
  ...
  // Split description by full stop and filter out any empty strings
  const descriptionPoints = work.description.split('.').filter(point => point.trim() !== '');

  return (
    <div className="space-y-6 py-6 first:pt-0 last:pb-0 border-b border-gray-300">
      <div>
        <div className="flex flex-wrap gap-2 gap-y-0 text-lg">
          <span className="font-semibold text-gray-800 dark:text-white">{work.position}</span>
          ...
          {work.company.website ? (
            <a
              ...
              className="group inline-flex items-center underline underline-offset-2 hover:no-underline text-blue-600"
            >
            ...
            <span className="font-medium text-gray-700 dark:text-gray-300">{work.company.name}</span>
          )}
        </div>
        <div className="space-x-3 text-sm text-neutral-500">
      ...
      {/* {work.tags && work.tags.length > 0 && (
        <div className="flex flex-wrap gap-2 mt-4">
          {work.tags.map((tag, index) => (
            <span
              key={index}
              className="inline-flex items-center px-3 py-1 text-sm font-medium rounded-full bg-gray-200 text-gray-800 dark:bg-gray-700 dark:text-gray-200 border border-gray-300 dark:border-gray-600"
            >
              {tag}
            </span>
          ))}
